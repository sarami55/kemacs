...
/*ARGSUSED*/
setcrkey(f, n)	/* reset encryption key of current buffer */
{
	...
	cryptstr(key, (unsigned)Cstrlen(key));

	/* and save it off */
	(void)Cstrcpy(curbp->b_key, key);
	mlwrite(CSTR(&Cxstr[5]));		/* clear it off the bottom line */
	return(TRUE);
}

/**********
 *
 *	cryptstr - in place encryption/decryption of a buffer
 *
 ...
 *	cryptstr().  First, you call 
 *
 *		cryptstr(NULL, vector)
 *
...
{
	register Char cc;	/* current character being considered */
