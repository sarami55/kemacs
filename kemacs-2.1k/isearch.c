...
Char	(*saved_get_char)();		/* Get character routine */
...
risearch(f, n)
{
    ...
    (void)backchar(TRUE, 1);			/* Back up a character		      */
...
isearch(f, n)
{
    ...
    register Char	c;		/* current input character */
    register Ckey	expc;		/* function expanded input char	      */
    ...
    (void)Cstrncpy (pat_save, pat, NPAT);	/* Save the old pattern string	      */
    curline = curwp->w_dotp;		/* Save the current line pointer      */
    curoff  = curwp->w_doto;		/* Save the current offset	      */
...
start_over:
    ...
    col = promptpattern(
#if KMSGS
		kterminal? CSTR(&Cxstr[3141]):
#endif
		CSTR(&Cxstr[3148]));
						/* Prompt, remember the col   */
    cpos = 0;					/* Start afresh		      */
	...
	if (c == IS_REVERSE) {			/* forward search?	      */
	    ...
	    (void)backchar(TRUE, 1);		/* Be defensive about EOB     */
	} else
	...
	switch (c)				/* dispatch on the input char */
	{
	  ...
	  case IS_RUBOUT:			/*  or if a Rubout:	      */
	    ...
	    (void)Cstrncpy (pat, pat_save, NPAT);	/* Restore the old search str */
	...
	int	dir;		/* Search direction			 */
{
    ...
    register Char buffchar;		/* character at current position      */
...
int
match_pat (patrn)	/* See if the pattern string matches string at "."   */
    Char	*patrn;	/* String to match to buffer			     */
{
    ...
    for (i = 0; i < Cstrlen(patrn); i++) /* Loop for all characters in patrn   */
    {
	if (curoff == llength(curline)) /* If at end of line		      */
	{
	    curline = lforw(curline);	/* Skip to the next line	      */
 ...
 * returns the number of column for prompt.
 */
    ...
    Char *prompt;
{
    ...
    (void)expandp(pat, &tpat[Cstrlen(tpat)], NPAT/2);	/* add old pattern */
    (void)Cstrcat(tpat, CSTR(&Cxstr[3200]));

    /* check to see if we are executing a command line */
    if (!clexec) {
	mlwrite(CSTR(&Cxstr[1281]), tpat);
    }
    for (p = tpat, w = 0; *p; p++)
    	w +=
#if KANJI
    ...
    int	col;	/* column to be echoed in */
{
	...
	switch (c)				/* Yes, dispatch special cases*/
	{
	  ...
	  case 0x7F:				/* Rubout:		      */
	    outstring(CSTR(&Cxstr[3210]));		/* Output a funny looking     */
						/*  indication of Rubout      */
...
get_char ()
{
    Ckey c;				/* A place to get a character	      */
