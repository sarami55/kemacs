...
scrnextup(f, n)		/* scroll the next window up (forward) a page */
{
...
scrnextdw(f, n)		/* scroll the next window down (back) a page */
{
	(void)nextwind(FALSE, 1);
	(void)backpage(f, n);
	(void)prevwind(FALSE, 1);
...
newwidth(f, n)	/* resize the screen, re-writing the screen */
{
	...
	if (n < 10 || n > term.t_mcol) {
		mlwrite(CSTR(&Cxstr[1281]),
#if KMSGS
			kterminal? CSTR(&Cxstr[7923]):
#endif
			CSTR(&Cxstr[7932]));
		return(FALSE);
	}
	return(changewidth(n));
}
changewidth(n)
{
...
change_win_size(SIGARG_T(dummy))
{
	...
	changewidth(col);
	update(TRUE);
	(void)signal(SIGWINCH, change_win_size);
}
#endif
