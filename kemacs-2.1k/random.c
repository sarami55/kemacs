	...
	int global;	/* true = global flag,	false = current buffer flag */
{
	...
	if (kterminal) {
		if (kind == TRUE)
			(void)Cstrcpy(prompt, CSTR(&Cxstr[6512]));
		else
			(void)Cstrcpy(prompt, CSTR(&Cxstr[6515]));
		(void)Cstrcat(prompt, CSTR(&Cxstr[6518]));
		if (global)
			(void)Cstrcat(prompt, CSTR(&Cxstr[6521]));
		(void)Cstrcat(prompt, CSTR(&Cxstr[6528]));
	} else {
#endif
	if (global)
		(void)Cstrcpy(prompt,CSTR(&Cxstr[6535]));
	else
		(void)Cstrcpy(prompt,CSTR(&Cxstr[6551]));

	if (kind == TRUE)
		(void)Cstrcat(prompt, CSTR(&Cxstr[6560]));
	else
		(void)Cstrcat(prompt, CSTR(&Cxstr[6566]));
#if KANJI
	}
#endif
