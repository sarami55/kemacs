...
# endif
	...
	(void)ffclose();
#endif
	(void)Cstrcpy(line, CSTR(&Cxstr[2561]));
	if (lflag)
		(void)Cstrcat(line,
#if KMSGS
				    kterminal? CSTR(&Cxstr[2563]):
#endif
				    CSTR(&Cxstr[2572]));
	if (s == FIOFUN)
		(void)Cstrcat(line,
#if KMSGS
				    kterminal? CSTR(&Cxstr[2593]):
#endif
				    CSTR(&Cxstr[2604]));
	if (s == FIOEOF || s == FIOFUN) {        /* Don't zap message!   */
		...
		if (kterminal) {
			...
			(void)Cstrcat(line, CSTR(&Cxstr[2624]));
		} else {
#endif
		(void)Cstrcat(line, CSTR(&Cxstr[2634]));
		(void)Cstrcat(line, itoa(nline));
		(void)Cstrcat(line, CSTR(&Cxstr[2640]));
		if (nline > 1)
			(void)Cstrcat(line, CSTR(&Cxstr[1147]));
#if KANJI
		}
#endif
		(void)Cstrcat(line, CSTR(&Cxstr[30]));
	}
	if (s != FIOERR)
		mlwrite(line);
	...
	Char	*fn;
{
	...
	if (s == FIOSUC) {			/* No write error.	*/
		...
		if (s == FIOSUC) {		/* No close error.	*/
			mlwrite(
#if KMSGS
			    kterminal? CSTR(&Cxstr[2783]):
#endif
			    CSTR(&Cxstr[2797]), nline,
			    (nline==1)? CSTR(&Cxstr[6]):
					CSTR(&Cxstr[1147]));
		}
...
filename(f, n)
{
	...
	if ((s=mlreply(
#if KMSGS
		kterminal? CSTR(&Cxstr[2815]):
#endif
		CSTR(&Cxstr[2826]),
				       fname, NFILEN)) == ABORT)
		return (s);
	(void)copystr(&curbp->b_fname, (s!=FALSE)? fname: CSTR(&Cxstr[6]));
	upmode();
...
Char	fname[];
{
	...
	if (s == FIOFUN)
		(void)Cstrcat(line, 
#if KMSGS
				    kterminal? CSTR(&Cxstr[2893]):
#endif
				    CSTR(&Cxstr[2604]));
	if (s == FIOEOF || s == FIOFUN) {        /* Don't zap message!   */
#if KANJI
		(void)Cstrcat(line, itoa(nline));
		(void)Cstrcat(line, CSTR(&Cxstr[2903]));
#else
		(void)Cstrcat(line, CSTR(&Cxstr[2912]));
		(void)Cstrcat(line, itoa(nline));
		(void)Cstrcat(line, CSTR(&Cxstr[2640]));
		if (nline > 1)
			(void)Cstrcat(line, CSTR(&Cxstr[1147]));
